...
float buttonCenterY = displayHeight/3.33f;
...
class lostPoint {
  ...
  public void draw() {
    ...
    text("-1",x,y);
    y += (height/720)*PApplet.parseInt(vel);
...
public void setup() {  // this is run once.   
    ...
    //boolean device = true;
    //if(device){
      size(PApplet.parseInt(displayWidth), PApplet.parseInt(displayWidth)); 
...
public void title() {
    ...
    textSize(70*scal);
    text("Alpha!",width/2,height/6 + ((96*scal)*1.25f));
    options.draw();
    credits.draw();
    play.draw();
