package processing.test.targo_tap;
...
public class Targo_tap extends PApplet {
...
class Prompt {
  ...
  public void draw() {
    ...
    fill(0);
    rect(round(width*0.1f)-(2*scalX),round(height/2) - round(round(height*0.3f)/2)-(2*scalY),round(width*0.8f)+(2*scalX),round(height*0.3f)+(2*scalY));
    fill(100);
    rect(round(width*0.1f)-(2*scalX),round(height/2) - round(round(height*0.3f)/2)-(2*scalY),round(width*0.8f)+(2*scalX),scalX*45);
    fill(255);
    textSize(scalX*40);
    text(this.title,round(width*0.1f),round(height/2) - round(round(height*0.3f)/2) + (scalY*3),round(width*0.8f),round(height*0.3f));
    textSize(scalX * 30);
    text(this.message,round(width*0.1f),round(height/2) - round(round(height*0.3f)/2) + (scalX*47),round(width*0.8f),round(height*0.3f));
...
int backgroundColor = color(216, 222, 191);
...
class Button{
    ...
    Button(int fntClr, int clr, int tx, int ty, int tw, float th, int tid){
        this.x = tx;
        this.y = ty;
        this.w = tw;
        this.h = th;
    ...
    public boolean checkPressed(){
        if(mouseX>x && mouseX<x+w && mouseY>y && mouseY<y+h){
             if(this.id+1 == goalNum){
               int oldGN = PApplet.parseInt(goalNum);
...
class menuButton{
    ...
    String txt;
    boolean isOn = false;
    menuButton(String txt, int clr, int fntClr, int x, int y, int w, int h){
        /*if(x=="mid"){
            x=width/2-w/2;
        }
        if(y=="mid"){
...
class lostPoint {
  ...
  public void draw() {
    this.vel += this.acc;
    if(round(((PApplet.parseFloat(lifetime - lifeStart)/1000)/2)*255) >= 256){
      this.shouldRemove = true;
    }
    lifetime = millis();
    textAlign(CENTER,CENTER);
    textSize(60*scal);
    fill(255,0,0,255 - round(((PApplet.parseFloat(lifetime - lifeStart)/1000)/2)*255));
    text("-1",x,y);
    this.y += (height/720)*PApplet.parseInt(vel);
...
public void setup() {  // this is run once.   
    ...
    // button definition
    button0 = new Button(color(8, 19, 166),color(149, 148, 209),0, height/5, width/2, height/3.33f,0);
    button1 = new Button(color(166, 8, 8),color(207, 147, 147),width/2, height/5, width/2, height/3.33f,1);
    button2 = new Button(color(166, 8, 8),color(207, 147, 147),0, height/2, width/2, height/3.33f,2);
    button3 = new Button(color(8, 19, 166),color(149, 148, 209),width/2, height/2, width/2, height/3.33f,3);
    
    //button4 = new Button(width/1.5, height/2,width/2,height/3.33,3);
    
    options = new menuButton("Options",color(149,211,206),color(7,166,153),0, height - round(height/3.33f)-1, width/2, round(height/3.33f));
    credits = new menuButton("Credits",color(252,179,126),color(239,113,24),width/2, height - round(height/3.33f)-1, width/2, round(height/3.33f));
    actionMode = new menuButton("Action Mode",color(239,113,24),color(252,179,126),0, height - (round(height/3.33f)*2), width/2, round(height/3.33f));
    zenMode = new menuButton("Zen Mode",color(149,211,206),color(7,166,153),width/2, height - (round(height/3.33f)*2), width/2, round(height/3.33f));
    restart = new menuButton("Restart",color(149,211,206),color(7,166,153),0,round(height/1.5f),width/2,height/6);
    menu_button = new menuButton("Menu",color(207,206,147),color(166,163,8),round(width/2),round(height/1.5f),width/2,height/6);
    play = new menuButton("Play Targo Tap",color(157, 209, 148),color(8, 166, 24),0, height - (round(height/3.33f)*2), width, round(height/3.33f));
    ...
    option_music = new menuButton("Music is NYI",color(157, 209, 148),color(8, 166, 24),round(width*0.1f),round(height/2) - round(round(height/6)/2),round(width*0.8f),height/6);
...
public void title() {
    ...
    textSize(70*scal);
    text("Fresh!",width/2,height/6 + ((96*scal)*1.25f));
    options.draw();
    credits.draw();
    play.draw();
...
public void buttonUpdate(Button button, int id){
  button.id = id;
}
int lastTime;
int startTime;
...
public void drawGame(){
    ...
    switch(gameState){
        ...
        case 2:
            ...
            background(backgroundColor);
            textAlign(CENTER,CENTER);
            text("Score: "+score,width/2,height/2.55f);
            text("Game Over",width/2,height/3.25f);
            restart.draw();
            menu_button.draw();
            break;
        case 3:
